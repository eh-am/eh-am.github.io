...
Bash (and shell scripting in general) is NOT straightforward. It's easy to mess up if you don't know what you are doing. If you come from traditional programming background and just want to plumb a few lines of code there are a few behaviours that will confuse the hell out of you.

To help wih that the [unnoficial bash strict mode](http://redsymbol.net/articles/unofficial-bash-strict-mode/) was created. In this post we will go over a misleading behaviour and how does the strict mode help (and its quirks).
...
Should it run all the way through, or should it fail, since the file does not exist?
...
Sounds reasonable. So why is "Fail silently" the normal behaviour in a shell script? Well, think that in a context of a shell you DO NOT want to exit when there's an error (imagine crashing your shell when you `cat` a file that does not exist). Looks like the behaviour was simply carried out to the non-interactive shell.
...
Which would then fail. Since the file does not exist, `cat` returns a non-zero exit code. The behaviour can be described in the following [bats](https://github.com/sstephenson/bats) unit test.
...
So given example below, the `echo` will never be run.
{{< file "/posts/bash-strict-mode/grep_fail.sh" "bash" >}}

So what can we do in that situation? Thankfully there's a bit in the bash manual on `errexit` section that can help us (reformatted for clarity):
...
As the command following the final `||` is `true`, there's no way the whole line to fail.
...
Yup. Once you are more familiar, read the entry on [BashFAQ](http://mywiki.wooledge.org/BashFAQ/105) and its subsequent links.
...
To put in other words, it will only return 0 if all parts of the pipeline return 0.
...
Let's fix example before
{{< file "/posts/bash-strict-mode/pipefail_first_correct.sh" "bash" >}}

Both behaviours can be verified by the following Bats test:
...
My recommendation for this case is to simply break down into different instructions:

{{< file "/posts/bash-strict-mode/pipefail_quirk_1_correct.sh" "bash" >}}

The behaviour can be confirmed by the following bats test:
...
In this example, we are loading a whitelist file and passing to another command (here implemented as a function) that would pass to yet another service (eg a CLI tool). Even though the file does not exist, the pipeline does not fail, ending up passing an empty string to `remove_hosts`, which could have catastrophic effects! (deleting more than you expect).

Ideally, you want to fail as soon as possible. The best way to do so is to break down into more instructions and just be more careful ¯\_(ツ)\_/¯

{{< file "/posts/bash-strict-mode/pipefail_quirk_2_correct.sh" "bash" >}}

As always, behaviour can be described by the following bats file:
